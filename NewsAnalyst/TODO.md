...
- [x] UTC 时区偏移 Bug 修复：DateNavigator 箭头切换日期后，非 UTC 时区用户查询日期偏移一天（本地午夜 ≠ UTC 午夜），导致切回今天后显示昨天文章；完全重写为 `Date.UTC()` 纯 UTC 运算
...
_最后更新：2026-03-09（UTC 时区偏移 Bug 修复；日期刷新持久化；竞态条件修复）_
