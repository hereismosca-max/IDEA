...
| ai_summary | TEXT | | AI 生成的 2-3 句客观摘要（GPT-4o-mini） |
| ai_tags | JSONB | | AI 结构化标签：`{"entities":[],"locations":[],"sectors":[],"topics":[],"scale":""}` |
| ai_score | FLOAT | | _(Phase 3 预留)_ AI 重要性评分（0.0 - 1.0） |
| ai_processed_at | TIMESTAMP | | AI 处理完成时间 |

> **注**：`ai_summary` 和 `ai_tags` 已在 v0.2.0 投入使用。`ai_score` 留 Phase 3 填充。
...
### 6. ArticleVotes（文章投票表）

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | UUID | PRIMARY KEY, DEFAULT gen_random_uuid() | 投票记录唯一标识 |
| user_id | UUID | NOT NULL, FK → Users.id ON DELETE CASCADE | 投票用户 |
| article_id | UUID | NOT NULL, FK → Articles.id ON DELETE CASCADE | 被投票文章 |
| vote | INTEGER | NOT NULL | `1` = 赞同（▲），`-1` = 反对（▼） |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | 投票时间 |

> **唯一约束**：`UNIQUE(user_id, article_id)`——每个用户对每篇文章只能有一条投票记录，切换方向时 UPDATE，再次同方向投票时 DELETE（toggle off）。

---

### 7. UserSavedArticles（用户收藏表）
...
_最后更新：2026-02-27（新增 ArticleVotes 表；Articles 表 ai_summary / ai_tags 字段投入使用）_
